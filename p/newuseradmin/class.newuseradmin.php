<?php
    class newuseradmin_plugin extends control{
        ...
        public function main(){ //return print( $this->location_to_select('loca', 89) );
            ...
            $model->addScript($model->pluginurl . 'newuseradmin.js', 'useradmin.js', 1 );            
?>
...
<script type="text/javascript">
   $("#grid1").flexigrid({
            ...
            colModel : [
                ...
                {display: "Kullanıcı Adı", name : "permalink", width : 150, sortable : true, align: "left"},
                {display: "Role", name : "role", width : 100, sortable : false, align: "center"},
                {display: "Profil", name : "status", width : 30, sortable : false, align: "center"},                               
                {display: "Action", name : "action", width : 200, sortable : false, align: "center"},
                {display: "Last Login IP", name : "ip", width : 150, sortable : false, align: "center"}
                ],                
            searchitems : [
                {display: "Name", name : "name", isdefault: true},
                {display: "Kullanıcı Adı", name : "permalink", isdefault: true},
...
<?php
        ...
        public function ajax_notVekil(){
            global $model, $db;
            $model->mode = 0;
			
            $ID = filter_input(INPUT_POST, 'ID', FILTER_SANITIZE_NUMBER_INT);
            $db->setQuery("SELECT p.ID, p.notVekil FROM profile AS p WHERE p.ID = " . $db->quote($ID)." LIMIT 1");
            $row = null;
            if($db->loadObject($row)){
                if($model->useris('superadmin') || ( $model->useris('admin') && !$model->useris('superadmin', $row->role))){
                    //if($row->status == 0 ) $row->status = -1;
        ...
        public function ajax_rows(){
            ...
            $SELECT = "SELECT u.*, p.name, p.permalink, p.lastLoginIP IP";
            $FROM   = "\n FROM user AS u";
            ...
            if($query){
                ...
                }elseif($qtype == 'permalink'){
                    $WHERE = "\n WHERE p.permalink LIKE '%".$db->escape($query)."%'";
                }else{
            ...
            if(count($rows)){
                foreach($rows as $row){
                    ...
                    if($model->useris('superadmin') || ( $model->useris('admin') && !$model->useris('superadmin', $row->role))){
                  	...
                  	$profil = '<a href="/userdetailadmin/'.$row->ID.'" target="_blank" title="active">Profil</a> ';
                    
                    $db->setQuery("select notVekil from profile where ID='".$row->ID."'"); 
            		$prfileVek = $db->loadResult();
            		//var_dump($prfileVek);
            		if($prfileVek==0)
            		$notVekil='<a href="#" rel="'.$row->ID.'" class="notVekil" title="block"><img src="'.PLUGINURL.'lib/icons/stop.png" alt="Vekil Yasağı Koy" border="0" /></a> ';
            		else 
            		$notVekil='<a href="#" rel="'.$row->ID.'" class="notVekil" title="block"><img src="'.PLUGINURL.'lib/icons/time.png" alt="Vekil Yasağını Kaldır" border="0" /></a> ';
