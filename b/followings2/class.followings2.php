<?php
    class followings2_block extends control{
        ...
        public function block(){
            ...
            $db->setQuery("SELECT ID, showfollowings FROM profile WHERE ID = " . $db->quote($myID) . " AND status>0" );
            $row = null;
            if($db->loadObject($row)){
                if(!profile::isallowed($row->ID, $row->showfollowings)) {
                    return;
                }
            } else {
                return;
...
?>
                        ...
                        <div class="box" id="following">
                            ...
                            <span class="all_users"><a href="/followings/<?=$myID?>">Hepsini Gör (<?=$count?>)</a></span>
                            <div class="clear"></div>
                            <div class="line"></div>
